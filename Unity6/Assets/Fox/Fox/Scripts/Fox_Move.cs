...
public class Fox_Move : MonoBehaviour {
	...
	void Movement(){
		...
		if(Input.GetKey(KeyCode.Z)){
			//Run
			rb.linearVelocity = new Vector2(move*speed*Time.deltaTime*3,rb.linearVelocity.y);
			running=true;
		}else{
			//Walk
		...
		//Movement Animation
		if(rb.linearVelocity.x!=0&&running==false){
			anim.SetBool("Walking",true);
		}else{
			anim.SetBool("Walking",false);
		}
		if(rb.linearVelocity.x!=0&&running==true){
	...
	void Jump(){
		//Jump
		if(Input.GetKeyDown(KeyCode.X)&&rb.linearVelocity.y==0){
			rb.AddForce(new Vector2(0,jumpForce));

		}
		//Jump Animation
		if(rb.linearVelocity.y>0&&up==false){
			up=true;
			jumping=true;
			anim.SetTrigger("Up");
		}else if(rb.linearVelocity.y<0&&down==false){
			down=true;
			jumping=true;
			anim.SetTrigger("Down");
		}else if(rb.linearVelocity.y==0&&(up==true||down==true)){
			up=false;
			down=false;
			jumping=false;
